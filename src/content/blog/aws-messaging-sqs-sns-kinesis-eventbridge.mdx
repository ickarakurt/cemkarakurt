...
<Mermaid client:visible code={`
flowchart LR
    Pub[Publisher] --> SNS{SNS Topic}
    SNS -->|Fanout| SQS1[SQS Queue 1]
    SNS -->|Fanout| SQS2[SQS Queue 2]
    SQS1 --> Sub1[Subscriber 1]
    SQS2 --> Sub2[Subscriber 2]
`} />
...
flowchart TD
    ...
    subgraph Services
        ...
        InvSvc[Inventory Management Service]
    ...
    Web -->|Place Order| SQS
    SQS --> OrderSvc
    OrderSvc -->|Save Order| DB
    
    OrderSvc -->|Publish Event| SNS
    SNS -->|Fanout| NotifSvc
    SNS -->|Fanout| InvSvc
    
    Web -->|Stream Activity| Kinesis
    Kinesis --> AnalyticsSvc
    
    OrderSvc -->|State Change| EB
    EB -->|Orchestrate| InvSvc
    EB -->|Trigger| NotifSvc
`} />
